...
int c_Solver::Init(int argc, char **argv) {
    ...
    if (nprocs != vct->getNprocs()) {
        if (myrank == 0) {
            cerr << "Error: " << nprocs << " processes cant be mapped into " << vct->getXLEN() << "x" <<
            vct->getYLEN() << "x" << vct->getZLEN() <<
            " matrix: Change XLEN,YLEN, ZLEN in method VCtopology3D.init()" << endl;
            mpi->finalize_mpi();
            return (1);
        }
    }
    ...
    for (int isat = 0; isat < nsat; isat++) {
        for (int jsat = 0; jsat < nsat; jsat++) {
            for (int ksat = 0; ksat < nsat; ksat++) {
                ...
                my_file << grid->getXC(index1, index2, index3) << "\t" << grid->getYC(index1, index2, index3) << "\t" <<
                grid->getZC(index1, index2, index3) << endl;
            }
        }
    }
    my_file.close();
...
void c_Solver::GatherMoments() {
    ...
    if (col->getCase() == "Dipole") {
        EMf->ConstantChargePlanet(grid, vct, col->getL_square(), col->getx_center(), col->gety_center(),
                                  col->getz_center());
    }
...
void c_Solver::UpdateCycleInfo(int cycle) {
    ...
    if(col->getCase() == "shock"){
        for(int i = 1; i < grid->getNXC() - 1; ++i){
            for(int k = 1; k < grid->getNZC() - 1; ++k){
                //todo reflected momentum from left side
                for(int partC = 0; partC < col->getNs(); ++partC){
                    theoreticalEnergy -= abs(col->getRHOinit(partC)/(fourPi*col->getQOM(partC)))*col->getV0(partC)*0.5*(sqr(col->getU0(partC)) + sqr(col->getV0(partC)) + sqr(col->getW0(partC)))*col->getDt()*col->getDx()*col->getDz();
                    theoreticalMomentumX -= abs(col->getRHOinit(partC)/(fourPi*col->getQOM(partC)))*col->getV0(partC)*col->getU0(partC)*col->getDt()*col->getDx()*col->getDz();
                    theoreticalMomentumY -= abs(col->getRHOinit(partC)/(fourPi*col->getQOM(partC)))*col->getV0(partC)*col->getV0(partC)*col->getDt()*col->getDx()*col->getDz();
                    theoreticalMomentumZ -= abs(col->getRHOinit(partC)/(fourPi*col->getQOM(partC)))*col->getV0(partC)*col->getW0(partC)*col->getDt()*col->getDx()*col->getDz();
                }
                theoreticalEnergy -= (col->getC()/fourPi)*(EMf->getEz(i, grid->getNYN()-2, k)*EMf->getBx(i, grid->getNYN()-2, k) -
                    EMf->getEx(i, grid->getNYN()-2, k)*EMf->getBz(i, grid->getNYN()-2, k))*col->getDt()*col->getDx()*col->getDz();
                theoreticalEnergy += (col->getC()/fourPi)*(EMf->getEz(i, 0, k)*EMf->getBx(i, 0, k) -
                                                                   EMf->getEx(i, 0, k)*EMf->getBz(i, 0, k))*col->getDt()*col->getDx()*col->getDz();


                theoreticalMomentumY -= (1.0/fourPi)*(EMf->getEz(i, grid->getNYN()-2, k)*EMf->getBx(i, grid->getNYN()-2, k) -
                                                                   EMf->getEx(i, grid->getNYN()-2, k)*EMf->getBz(i, grid->getNYN()-2, k))*col->getDt()*col->getDx()*col->getDz();
                theoreticalMomentumY += (1.0/fourPi)*(EMf->getEz(i, 0, k)*EMf->getBx(i, 0, k) -
                                                                   EMf->getEx(i, 0, k)*EMf->getBz(i, 0, k))*col->getDt()*col->getDx()*col->getDz();
            }
        }
    }
...
void c_Solver::InjectBoundaryParticles() {
    ...
    for (int i = 0; i < numberSpecies; i++) {
        if (col->getRHOinject(i) > 0.0) {
            ...
            if (col->getCase() == "Dipole") {
                for (int i = 0; i < numberSpecies; i++)
                    Qremoved[i] = part[i].deleteParticlesInsideSphere(col->getL_square(), col->getx_center(),
                                                                      col->gety_center(), col->getz_center());
...
void c_Solver::WriteConserved(int cycle) {
    ...
    if (cycle % col->getDiagnosticsOutputCycle() == 0) {
        ...
        if (myrank == 0) {
            ...
            my_file << cycle << "\t" << "\t" << (Eenergy + Benergy + TOTenergy) << "\t" << TOTmomentum << "\t" <<
            Eenergy << "\t" << Benergy << "\t" << TOTenergy << endl;
            my_file.close();
        }
...
void c_Solver::WriteOutput(int cycle) {
    ...
    if (col->getWriteMethod() == "h5hut") {
        ...
        if (cycle % (col->getParticlesOutputCycle()) == 0 &&
            cycle != col->getLast_cycle() && cycle != 0)
            WritePartclH5hut(numberSpecies, grid, part, col, vct, cycle);
            ...
            for (int isat = 0; isat < nsat; isat++) {
                for (int jsat = 0; jsat < nsat; jsat++) {
                    for (int ksat = 0; ksat < nsat; ksat++) {
                        ...
                        int index2 = 1 + jsat * ny0 / nsat + ny0 / nsat / 2;
                        int index3 = 1 + ksat * nz0 / nsat + nz0 / nsat / 2;
                        my_file << EMf->getBx(index1, index2, index3) << "\t" << EMf->getBy(index1, index2, index3) <<
                        "\t" << EMf->getBz(index1, index2, index3) << "\t";
                        my_file << EMf->getEx(index1, index2, index3) << "\t" << EMf->getEy(index1, index2, index3) <<
                        "\t" << EMf->getEz(index1, index2, index3) << "\t";
                        my_file << EMf->getJxs(index1, index2, index3, 0) + EMf->getJxs(index1, index2, index3, 2) <<
                        "\t" << EMf->getJys(index1, index2, index3, 0) + EMf->getJys(index1, index2, index3, 2) <<
                        "\t" << EMf->getJzs(index1, index2, index3, 0) + EMf->getJzs(index1, index2, index3, 2) << "\t";
                        my_file << EMf->getJxs(index1, index2, index3, 1) + EMf->getJxs(index1, index2, index3, 3) <<
                        "\t" << EMf->getJys(index1, index2, index3, 1) + EMf->getJys(index1, index2, index3, 3) <<
                        "\t" << EMf->getJzs(index1, index2, index3, 1) + EMf->getJzs(index1, index2, index3, 3) << "\t";
                        my_file <<
                        EMf->getRHOns(index1, index2, index3, 0) + EMf->getRHOns(index1, index2, index3, 2) << "\t";
                        my_file <<
                        EMf->getRHOns(index1, index2, index3, 1) + EMf->getRHOns(index1, index2, index3, 3) << "\t";
                    }
                }
            }
            my_file << endl;
...
void c_Solver::WriteSimpleOutput(int cycle) {
    ...
    if (cycle % 50 == 0) {
        ...
        for (int i = 1; i < grid->getNXN() - 1; ++i) {
            for (int j = 1; j < grid->getNYN() - 1; ++j) {
                for (int k = 1; k < grid->getNZN() - 1; ++k) {
                    ...
                    fprintf(velocityElectronFile, "%g %g %g\n", EMf->getJxs(i, j, k, 0) / electronChargeDensity,
                            EMf->getJys(i, j, k, 0) / electronChargeDensity,
                            EMf->getJzs(i, j, k, 0) / electronChargeDensity);
                    fprintf(velocityProtonFile, "%g %g %g\n", EMf->getJxs(i, j, k, 1) / protonChargeDensity,
                            EMf->getJys(i, j, k, 1) / protonChargeDensity,
                            EMf->getJzs(i, j, k, 1) / protonChargeDensity);
                }
            }
        }
        ...
        for (int i = 0; i < part[0].getNOP(); ++i) {
            fprintf(electronsFile, "%20.15g %20.15g %20.15g\n", part[0].getY(i), part[0].getP(i), part[0].getPy(i));
        }

        for (int i = 0; i < part[1].getNOP(); ++i) {
            fprintf(protonsFile, "%20.15g %20.15g %20.15g\n", part[1].getY(i), part[1].getP(i), part[1].getPy(i));
        }
        ...
        if(cycle == 0){
            ...
            theoreticalMomentumX = momentumX;
            theoreticalMomentumY = momentumY;
            theoreticalMomentumZ = momentumZ;
        }
        //todo momentum - vector and theoretical
        fprintf(generalFile,
                "%d %15.10g %15.10g %15.10g %15.10g %15.10g %15.10g %15.10g %15.10g %15.10g %15.10g %15.10g %15.10g %15.10g %15.10g %15.10g %15.10g %d\n",
                cycle, simulation_time, simulation_time, kinEnergy, Eenergy,
                Benergy, energy, momentumX, momentumY, momentumZ,
                theoreticalEnergy, theoreticalMomentumX, theoreticalMomentumY, theoreticalMomentumZ, maxE, maxB, EMf->getDt(), numberOfParticles);
        //printf("%15.10g\n", EMf->getDt());
        fclose(generalFile);
...
void c_Solver::Finalize() {
    if (mem_avail == 0) {          // write the restart only if the simulation finished succesfully
        if (col->getWriteMethod() != "h5hut") {
            writeRESTART(RestartDirName, myrank, (col->getNcycles() + first_cycle) - 1, numberSpecies, mpi, vct, col,
                         grid, EMf, part, 0);
        }
    }
